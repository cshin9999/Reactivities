        ...
        <Tab.Pane>
            <Grid>
                <Grid.Column width={16} style={{ paddingBottom: 0 }}>
                    ...
                    {isCurrentUser && (
                        <Button onClick={() => setAddPhotoMode(!addPhotoMode)} floated='right' basic content={addPhotoMode ? 'Cancel' : 'Add Photo'} />
                    )}
                </Grid.Column>
                                ...
                                <Card key={photo.id}>
                                    ...
                                    {isCurrentUser && (
                                        <Button.Group fluid widths={2}>
                                            <Button onClick={e => { setMainPhoto(photo); setTarget(e.currentTarget.name); }} name={photo.id} disabled={photo.isMain} loading={loading && target === photo.id} basic positive content='Main' />
